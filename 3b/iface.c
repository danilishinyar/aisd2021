...
int D_Find_ks2(Table* ptb){
  ...
  while(ptr){
    if(!strcmp(key2, ptr->key))
      ...
      fread(&key2_len, sizeof(int), 1, ptb->item_fd);
        fread(&info_len, sizeof(int), 1, ptb->item_fd);
        fseek(ptb->item_fd, key2_len, SEEK_CUR);
      info = (char*)calloc(info_len, 1);
        fread(info, sizeof(char), info_len, ptb->item_fd);
        printf("%s(%d): %s\n", key2, ptr->release, info);
    ptr = ptr->next;
      free(key2);
      free(info);
  }
...
int D_Find_ks2_release(Table* ptb){
  ...
  if(ptr == NULL){
    ...
    return 0;
      ...
      //key1/key2_len/info_len/key2/info
      int key1 = 0, info_len = 0, key2_len = 0;
      ...
      char* info = (char*)calloc(info_len, 1);
      fread(info, sizeof(char), info_len, ptb->item_fd);
    printf("(%d, %s(%d)): %s", key1, key2, release, info);
    free(key2);
      free(info);
    return 1;
    }
...
int D_Find_ks1ks2(Table* ptb){
  ...
  if(ptr == NULL){
      ...
      int info_len = 0, key2_len = 0;
      int offset = ptr->offset_item + 4;
      fseek(ptb->item_fd, offset, SEEK_SET);
      fread(&key2_len, sizeof(int), 1, ptb->item_fd);
      fread(&info_len, sizeof(int), 1, ptb->item_fd);
      fseek(ptb->item_fd, key2_len, SEEK_CUR);
      info = (char*)calloc(info_len, 1);
      fread(info, sizeof(char), info_len, ptb->item_fd);
    printf("(%d, %s): %s", key1, key2, info);
    free(info);
    free(key2);
        return 1;
  }
...
int D_Show(Table* ptb){
    int key1 = 0, info_len = 0, key2_len = 0, release;
    char* info = NULL;
    char* key2 = NULL;
    for(int i = 0; i < ptb->csize1; ++i){
        ...
        fread(key2, sizeof(char), key2_len, ptb->item_fd);
        fread(info, sizeof(char), info_len, ptb->item_fd);
        release = find_release(ptb, key1, key2);
